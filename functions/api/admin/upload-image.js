...
export async function onRequestPost(context) {
  try {
    if (!context.env.MEDIA) {
      return json(
        { error: "Binding R2 MEDIA não configurado." },
        { status: 503 }
      );
    }
    ...
    const key =
      `uploads/${year}/${month}/${crypto.randomUUID()}.${extension}`;
    ...
    await context.env.MEDIA.put(
      ...
      await file.arrayBuffer(),
      {
        ...
        customMetadata: {
          uploadedBy:
            context.data.accessUser?.email || "unknown",
          originalName:
            String(file.name || "image").slice(0, 180)
        }
      }
    );
